...
/**
 * Offline integration tests for commands/ and public/index.php
...
// =====================================================
// SECTION 1: public/index.php flow
// =====================================================
echo "=== Section 1: public/index.php webhook flow ===\n\n";

$secretService = new AutoSecretByTokenService();

// Test 1.1: IP Validator (used in public/index.php)
...
// Test 1.2: Secret generation and validation (used in public/index.php)
...
// Test 1.3: Webhook payload processing (public/index.php core logic)
echo "\nTest 1.3: Webhook payload processing (index.php core)\n";
...
// Test 1.4: Invalid webhook secret (public/index.php validation)
...
// Test 1.5: Null secret (public/index.php validation)
...
// =====================================================
// SECTION 2: commands/poller.php flow
// =====================================================
echo "\n=== Section 2: commands/poller.php flow ===\n\n";

// Test 2.1: BotSecretDTO (used in poller.php)
...
// Test 2.2: BotSecretRegistry (used in poller.php)
...
// Test 2.3: TgUpdateProcessor with registry (used in poller.php)
echo "\nTest 2.3: TgUpdateProcessor with registry\n";
...
$processor = new TgUpdateProcessor($registry3);
$updateDTO = $webhook->build(telegram_webhook_payload());
...
// Test 2.4: Multiple updates processed (poller.php loop simulation)
...
$updates = [
    ['update_id' => 1, 'message' => ['message_id' => 1, 'from' => ['id' => 1, 'is_bot' => false, 'first_name' => 'A'], 'chat' => ['id' => 1, 'type' => 'private'], 'date' => 1000, 'text' => 'First']],
    ['update_id' => 2, 'message' => ['message_id' => 2, 'from' => ['id' => 1, 'is_bot' => false, 'first_name' => 'A'], 'chat' => ['id' => 1, 'type' => 'private'], 'date' => 1001, 'text' => 'Second']],
    ['update_id' => 3, 'message' => ['message_id' => 3, 'from' => ['id' => 1, 'is_bot' => false, 'first_name' => 'A'], 'chat' => ['id' => 1, 'type' => 'private'], 'date' => 1002, 'text' => 'Third']],
];
...
// =====================================================
// SECTION 3: commands/webhook.php flow
// =====================================================
echo "\n=== Section 3: commands/webhook.php flow ===\n\n";

// Test 3.1: AutoSecretByTokenService (used in webhook.php)
...
foreach ($invalidSecrets as $invalidSecret) {
    try {
        ...
        assert_test("secret '".var_export($invalidSecret, true)."' should throw", false, 'Expected exception not thrown');
    } catch (\BAGArt\TelegramBot\Exceptions\TgBotInvalidSecretException $e) {
