...
if (!$token) {
    $logger->error('Token not set. Use --token=xxx:xxx or "export TELEGRAM_BOT_TOKEN=xxx:xxx"');
    exit(1);
}
...
$rawClient = new TgBotApiClient($botSetup->transport);
...
while (true) {
    try {
            ...
            foreach ($response['result'] ?? [] as $update) {
                ...
                if ($echo && isset($update['message']['text'])) {
                    try {
                        ...
                        if (!is_array($sendMessageResponse) || !$sendMessageResponse['ok'] ?? null) {
                            $logger->error(
                                "tg api sendMessage response:"
                                .json_encode($sendMessageResponse, JSON_PRETTY_PRINT)
                            );
                            echo '-';
                        }
                    } catch (TgApiReturnException $e) {
                        $logger->error("tg api sendMessage response ".$e::class.": ".$e->getMessage());
                        echo '!';
                    }
                }
            }
        ...
        $logger->error("Tg api getUpdates response ".$e::class.": {$e->getMessage()}");
        echo 'E';
    }
