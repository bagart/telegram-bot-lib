        ...
        foreach ($schema['types'] as $dto => $dtoMeta) {
            $types[$dto] = [
                ...
                'description' => $this->getDescription(
                    $dtoMeta['description']['markdown'] ?? null
                ),
...
PHP;
    ...
    private function generateDtoClass(
        ...
        $dtoDescription = $this->getDescription(
            $dtoMeta['description']['markdown'] ?? null
        );
        $todo = [];
        if (!empty($dtoMeta['oneOf'])) {
            echo "[WARNING] $classname is contract oneOf. Not implemented yet.\n";
            $todo['oneOf'] = "#[Todo('Is oneOf contract. Not implemented yet.')]";
        }
        ...
        if (!empty($dtoMeta[$fieldsProperty])) {
            ...
            foreach ($dtoMeta[$fieldsProperty] as $fieldMeta) {
                ...
                $propertyDescription = $this->getDescription(
                    $fieldMeta['description']['markdown'] ?? null
                );
                ...
                if (count($literals) === 1) {
                    if (count($phpTypes) !== 1) {
                        throw new RuntimeException(
                            'Unexpected default type count:'
                            .json_encode($fieldMeta)
                        );
                    }
                    ...
                    if (count($phpTypes) !== 1) {
                        throw new RuntimeException(
                            'Unexpected enum field type count:'
                            .json_encode($fieldMeta)
                        );
                    }
...
PHP;
    ...
    private function getTypes(array $fieldMeta, bool $required = true): array
    {
        ...
        foreach ($types as $typeNode) {
            if (empty($typeNode['type']) || !is_string($typeNode['type'])) {
                throw new RuntimeException(
                    'Invalid field type: '.json_encode($fieldMeta)
                );
            }
        ...
        if (count($phpTypes) === 2 && isset($phpTypes['string'])) {
            if (isset($phpTypes['int'])) {
                ...
                throw new RuntimeException(
                    'Unexpected TYPE combination: '.json_encode($phpTypes)
                );
            }
        }
        ksort($phpTypes);
        ...
        if ($type === '') {
            throw new RuntimeException(
                'Unexpected field type:'.json_encode($fieldMeta)
            );
        }
        if (count($phpTypes) === 1 && $nullable) {
            $type = "?$type";
        } elseif ($nullable) {
            $type .= '|null';
        }
    ...
    private function resolveType(array $typeNode): string|array
    {
        ...
        return match ($typeNode['type']) {
            ...
            'union' => 'mixed',
            default => throw new RuntimeException(
                'Unexpected TYPE from:'.json_encode($typeNode)
            )
        };
    }
    ...
    private function generateEnum(
        ...
        foreach ($literals as $v) {
            ...
            if ($propOrigName === 'active_period') {
                if (!is_numeric($v)) {
                    throw new RuntimeException(
                        "Invalid value for active_period: $propOrigName"
                    );
                }
                if (0 === ($v % (24 * 60 * 60))) {
                ...
                $dto === 'giftPremiumSubscription'
                && $propOrigName === 'star_count'
            ) {
                $case = match ($v) {
                    1000 => 'STARS_3M',
                    1500 => 'STARS_6M',
...
PHP;
        ...
        $resultPath = str_replace(
            strtr(realpath(dirname(__DIR__, 6)), '\\', '/').'/',
            '',
            strtr($fileName, '\\', '/'),
        );
        if (file_exists($fileName)) {
            $existing = file_get_contents($fileName);
            if ($existing === $code) {
                $this->result['Actual'][$entityName.'Enum'][] = $resultPath;
            } else {
                $this->result['Updated'][$entityName.'Enum'][] = $resultPath;
            }
        } else {
            $this->result['Created'][$entityName.'Enum'][] = $resultPath;
        }
        ...
        if (file_exists($fileName) && !in_array($propOrigName, [
            ...
            if ($this->full) {
                $fileName = "{$this->dtoDir}/{$entityName}s/Enum/{$enumClassName}NeedToCheck_"
                    .random_int(10000, 99999).'.php';
    ...
    private function generateDtoListEnum(
        array $dtoList,
        string $tgApiEntityScopes,
    ): string {
        $cases = [];
...
if (php_sapi_name() === 'cli') {
    ...
    $jsonPath = dirname(__DIR__, 2).'/tg-bots-api.json';
    ...
    $result = array_map(
        fn ($types) => array_map(fn ($files) => count($files), $types),
        $generator->generate()
    );
